...
class OrderSellHandler extends OrderHandlers {
	public void exchange(PrintWriter out) throws IOException{
		...
		Order curr = new Order(user, symb, price, quantity, type, OrderCommand.SELL);
		...
		switch (type) {
			case GTC:
				symb.GTC_Exchange(curr, out);
				break;
			case IOC:
				symb.IOC_Exchange(curr, out);
				break;
			case MPO:
				symb.MPO_Exchange(curr, out);
				break;
			default:
				symb.GTC_Exchange(curr, out);						
		}
...
class OrderBuyHandler extends OrderHandlers {
	public void exchange(PrintWriter out) throws IOException{
		...
		Order curr = new Order(user, symb, price, quantity, type, OrderCommand.BUY);
		user.addOrder(curr);
